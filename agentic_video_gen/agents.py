...
def _build_manim_prompt(lang_cfg: LanguageConfig) -> str:
    return (
        ...
        "Your input contains a text prompt with five fields, optionally followed by map PNG images:\n"
        ...
        "  - [Optional] Attached images: the actual PNG renders of any map assets, in the same order as "
        "the map entries in 'Available SVG Assets'. Use these images to determine accurate coordinate "
        "placement for objects drawn over the maps (see rule 13).\n\n"
        ...
        "6b. Arabic text animations — NEVER use Write() or DrawBorderThenFill() on Arabic Text() objects. "
        "The Write() animation traces glyph strokes; Arabic glyphs with tashkeel produce complex stroke paths "
        "that manimpango cannot resolve, so the text renders invisible. "
        "Always use FadeIn() for Arabic Text() objects (and for VGroups that contain Arabic Text()). "
        "You may still use Write() on purely Latin/digit Text() objects.\n"
        ...
        "13. Map image visual context — IMPORTANT:\n"
        "   - The prompt may be accompanied by one or more map PNG images (attached as images after the text).\n"
        "   - These images show the ACTUAL geographic layout that will appear on screen at full-frame size.\n"
        "   - When placing Manim objects over a map (arrows, paths, labels, ships, dots, etc.), "
        "study the attached image to determine accurate Manim coordinates.\n"
        "   - The map fills x ∈ [-7.11, 7.11], y ∈ [-4.0, 4.0]. Identify the pixel position of key "
        "geographic features in the image (top-left origin) and convert: "
        "x_manim = (px / img_width - 0.5) * 14.22, y_manim = (0.5 - py / img_height) * 8.0.\n"
        "   - For CurvedArrow / ArcBetweenPoints paths over a map, pick start/end points that lie "
        "WITHIN the visible map region (|x| < 6.5, |y| < 3.5), and use a modest arc angle "
        "(|angle| ≤ PI * 0.7) to keep the arc within the frame.\n"
        "   - NEVER use coordinates outside x ∈ [-6.5, 6.5], y ∈ [-3.5, 3.5] for objects "
        "placed over a map."
    )
